...
class BOARD:
    ...
    def first_second_move(self, user_turn):
        ...
        if (user_turn):
            # give instructions
            coord1 = random.choice(first_moves)

            # remove this piece from the board
            print "User removes " + str(coord1)
            self.board[coord1[0]][coord1[1]] = ' '
            self.print_board(self.board)

            # computer goes second, chooses a random legal move
            second_moves = self.possible_second_moves(coord1)
    ...
    def get_move(self, user_turn, possible_moves, who_first):
        ...
        if (user_turn):
            ...
            if who_first == 'User':
                first_node = Node(self.board, None, 0, 4, 'X', who_first)          # 4 is the depth limit
            else:
                first_node = Node(self.board, None, 0, 4, 'O', who_first)
